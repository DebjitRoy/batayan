...
export interface CreatePostInput {
  title: string;
  postType: string;
  gist?: string;  // Now optional - generated by backend AI
...
export async function createPost(input: CreatePostInput, token: string): Promise<CreatePostResponse> {
  ...
  const response = await requestJson<any>('/posts', {
    method: 'POST',
    headers: authHeaders(token),
    body: JSON.stringify({
      title: input.title,
  ...
  return {
    post: mapPost(currentPost),
    summaryJobId: response.summaryJobId,
    grammerJobId: response.grammerJobId
  };
}
...
export async function updatePost(postId: string, input: CreatePostInput, token: string): Promise<UpdatePostResponse> {
  let seriesId = input.seriesId;
  ...
  const response = await requestJson<any>(`/posts/${postId}`, {
    method: 'PUT',
    headers: authHeaders(token),
    body: JSON.stringify({
      title: input.title,
